...
const query_default = groq`*[_type == "rule" && ( name match $query + "*" || group->name match $query + "*") ][0...10]{ 
	_id, 
	...
	const query_2 = groq`*[_type == "rule" && ( name match $query + "*" || group->name match $query + "*") ][0...10]{ 
		_id, 
		description,
		"name" : coalesce(group->name + " :: ", "") + name ,
		"group": group->name,
