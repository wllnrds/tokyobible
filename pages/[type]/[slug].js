...
function Block({ data, isRoot = false, extra = 0 }){
    return (<section className={ styles.block }>
        ...
        { data.rules && <section className={ styles.list }>
            <hr />
            { data.rules.map( sub => <Block data={ sub } extra={ data.value } /> ) }
        </section> }
        { data.origin && <footer className={ styles.footer }>{ data.origin.map( item => <div>{ item.source } p.{ item.page }</div>) }</footer> }
    </section>)
}
